...
$linkkembali = " <hr> <a class='btn btn-danger' href='delivery_mmki.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls') && $_FILES['excelfile']['size'] > 0) {
          ...
          if ($R) {
            echo "<script>
                        $(document).ready(function() {
                            ...
                            function(){
                                window.location.href = 'import_delivery_mmki.php';
                            });
                        });
                        </script>";
          } else if ($a == '' || $b == '' || $c == '' || $d == '' || $e == '') {
            ...
            $sql = "INSERT INTO tbl_deliverynote (dn_no, job_no, customerpart_no, qty_pcs, tanggal_order, plan, cycle, ETA, user) 
    ...
    if ($query) {
      header("Location: ../delivery_mmki.php?val=ok");
      exit();
    } else {
      header("Location: ../delivery_mmki.php?val=no");
      exit();
    }
