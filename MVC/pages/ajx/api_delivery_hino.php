<?php
// File: api_delivery_hino.php (Optimized)
...
$totalQuery = "SELECT COUNT(*) as total FROM tbl_deliveryhino a LEFT JOIN masterpart_hino b ON a.job_no = b.JobNo $whereSql";
...
$mainQuery = "
    SELECT
        ...
        ROUND(a.qty_pcs / b.QtyPerKbn) AS sequence_hino,
        b.QtyPerKbn AS qty_hino,
        (
    ...
    FROM tbl_deliveryhino a
    LEFT JOIN masterpart_hino b ON a.job_no = b.JobNo
    $whereSql
    ORDER BY a.id DESC
    LIMIT {$start}, {$length}
";
...
while ($d = mysqli_fetch_assoc($result)) {
    $statusLabel = ($d['sequence_hino'] == $d['countp']) ? '<label style="color:red;">Close</label>' : '<label>' . $d['status'] . '</label>';
