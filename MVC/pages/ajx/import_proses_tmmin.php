...
$linkkembali = " <hr> <a class='btn btn-danger' href='masterpart_tmmin.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls')
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
      foreach ($sheet->getRowIterator() as $row) {
        if ($count > 0) {
          ...
          $sql = "INSERT INTO masterpart_tmmin (InvId, PartName, PartNo, JobNo, QtyPerKbn, ModelNo, user) 
                                VALUES ('$a','$c', '$b', '$d', '$e', '$f', '$IdUser' )";
    ...
    if ($query) {
      header("Location: ../masterpart_tmmin.php");
      exit();
    } else {
