...
$linkkembali = " <hr> <a class='btn btn-danger' href='masterpart_hino.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls')
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
      foreach ($sheet->getRowIterator() as $row) {
        if ($count > 0) {
          ...
          $sql = "INSERT INTO masterpart_hino (InvId, PartName, PartNo, JobNo, QtyPerKbn, ModelNo, user) 
    ...
    if ($query) {
      header("Location: ../masterpart_hino.php");
      exit();
    } else {
