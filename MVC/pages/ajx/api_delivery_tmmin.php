<?php
// File: api_delivery_tmmin.php (Optimized)
...
$totalQuery = "SELECT COUNT(*) as total FROM tbl_deliverytmmin a LEFT JOIN masterpart_tmmin b ON a.job_no = b.JobNo $whereSql";
...
$mainQuery = "
    SELECT
        ...
        ROUND(a.qty_pcs / b.QtyPerKbn) AS sequence_tmmin,
        b.QtyPerKbn AS qty_tmmin,
        (
    ...
    FROM tbl_deliverytmmin a
    LEFT JOIN masterpart_tmmin b ON a.job_no = b.JobNo
    $whereSql
    ORDER BY a.id DESC
    LIMIT {$start}, {$length}
";
...
while ($d = mysqli_fetch_assoc($result)) {
    $statusLabel = ($d['sequence_tmmin'] == $d['countp']) ? '<label style="color:red;">Close</label>' : '<label>' . $d['status'] . '</label>';
