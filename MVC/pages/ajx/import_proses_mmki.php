...
$linkkembali = " <hr> <a class='btn btn-danger' href='masterpart_mmki.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls')
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
      foreach ($sheet->getRowIterator() as $row) {
        if ($count > 0) {
          ...
          $sql = "INSERT INTO masterpart_mmki (InvId, PartName, PartNo, JobNo, QtyPerKbn, ModelNo, user) 
    ...
    if ($query) {
      header("Location: ../masterpart_mmki.php");
      exit();
    } else {
