...
$linkkembali = " <hr> <a class='btn btn-danger' href='ekspor_adm.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls') && $_FILES['excelfile']['size'] > 0) {
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
      foreach ($sheet->getRowIterator() as $row) {
        if ($count > 0) {
          ...
          $f  = isset($row[5]) ? $row[5] : ''; // module_no
          $g  = isset($row[6]) ? $row[6] : ''; // cycle
          $h  = isset($row[7]) ? $row[7] : ''; // ETA

          // Tangani nilai kosong pada kolom job_no
          if ($b == '') {
              $b = NULL; // jika job_no kosong, set NULL
          }

          // Tangani nilai kosong pada kolom ETA
          if ($h == '') {
              $h = NULL; // jika ETA kosong, set NULL
          }

          $Q = mysqli_query($mysqli, "SELECT * FROM tbl_eksporadm WHERE customerpart_no='$c' AND module_no='$f'");
          ...
          if ($R) {
            echo "<script>
                        $(document).ready(function() {
                            ...
                            function(){
                                window.location.href = 'import_export_adm.php';
                            });
                        });
                        </script>";
          } else if ($a == '' || $c == '' || $d == '' || $e == '') {
            ...
            $sql = "INSERT INTO tbl_eksporadm (dn_no, job_no, customerpart_no, qty_pcs, tanggal_order, module_no, cycle, ETA, user) 
    ...
    if ($query) {
      header("Location: ../ekspor_adm.php?val=ok");
      exit();
    } else {
      header("Location: ../ekspor_adm.php?val=no");
      exit();
    }
