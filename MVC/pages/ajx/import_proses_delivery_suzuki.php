...
$linkkembali = " <hr> <a class='btn btn-danger' href='delivery_suzuki.php'>Kembali</a>";
...
if (!empty($_FILES['excelfile']['name'])) {
  ...
  if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls') && $_FILES['excelfile']['size'] > 0) {
    ...
    foreach ($reader->getSheetIterator() as $sheet) {
      foreach ($sheet->getRowIterator() as $row) {
        if ($count > 0) {
          ...
          $Q = mysqli_query($mysqli, "SELECT * FROM tbl_deliverysuzuki WHERE dn_no='$a' AND job_no='$b'");
          ...
          if ($R) {
            echo "<script>
                        $(document).ready(function() {
                            ...
                            function(){
                                window.location.href = 'import_delivery_suzuki.php';
                            });
                        });
                        </script>";
          } else if ($a == '' || $b == '' || $c == '' || $d == '' || $e == '') {
            ...
            $sql = "INSERT INTO tbl_deliverysuzuki (dn_no, job_no, customerpart_no, qty_pcs, tanggal_order, plan, cycle, ETA, user) 
    ...
    if ($query) {
      header("Location: ../delivery_suzuki.php?val=ok");
      exit();
    } else {
      header("Location: ../delivery_suzuki.php?val=no");
      exit();
    }
