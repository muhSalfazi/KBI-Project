<?php
// File: api_delivery_mmki.php (Optimized)
...
$totalQuery = "SELECT COUNT(*) as total FROM tbl_deliverynote a LEFT JOIN masterpart_mmki b ON a.job_no = b.JobNo $whereSql";
...
$mainQuery = "
    SELECT
        ...
        ROUND(a.qty_pcs / b.QtyPerKbn) AS sequence_mmki,
        b.QtyPerKbn AS qty_mmki,
        (
...
while ($d = mysqli_fetch_assoc($result)) {
    $statusLabel = ($d['sequence_mmki'] == $d['countp']) ? '<label style="color:red;">Close</label>' : '<label>' . $d['status'] . '</label>';
