...
class XMLReader extends \XMLReader
{
    ...
    public static function open($URI, $encoding = null, $options = 0)
    {
        ...
        $realPathURI = self::convertURIToUseRealPath($URI);

        // HHVM does not check if file exists within zip file
        // @link https://github.com/facebook/hhvm/issues/5779
        if (self::isRunningHHVM() && self::isZipStream($realPathURI)) {
            if (self::fileExistsWithinZip($realPathURI)) {
     ...
     */
    protected static function convertURIToUseRealPath($URI)
    {
        $realPathURI = $URI;

        if (self::isZipStream($URI)) {
     ...
     */
    protected static function isZipStream($URI)
    {
     ...
     */
    protected static function isRunningHHVM()
    {
     ...
     */
    protected static function fileExistsWithinZip($zipStreamURI)
    {
